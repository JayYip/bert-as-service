...
def optimize_graph(args, logger=None):
    ...
    try:
        ...
        init_checkpoint = os.path.join(
            args.tuned_model_dir or args.model_dir)
        ...
        input_ids = tf.placeholder(
            tf.int32, (None, args.max_seq_len), 'input_ids')
        input_mask = tf.placeholder(
            tf.int32, (None, args.max_seq_len), 'input_mask')
        input_type_ids = tf.placeholder(
            tf.int32, (None, args.max_seq_len), 'segment_ids')
        ...
        with jit_scope():
            ...
            # def masked_reduce_max(x, m): return tf.reduce_max(
            #     minus_mask(x, m), axis=1)
